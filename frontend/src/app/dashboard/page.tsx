...
interface Project {
  ...
  updated_at: string;
  latest_job_status: string | null; // For status badges if needed
}
...
function DashboardIcon({ className = "" }: { className?: string }) {
  return <svg className={`w-5 h-5 ${className}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" /></svg>;
}
...
function ReviewIcon({ className = "" }: { className?: string }) {
  return <svg className={`w-5 h-5 ${className}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>;
}
...
const navItems = [
  ...
  { id: "ai-agent", label: "مساعد الذكاء الاصطناعي", icon: SparkleIcon, href: "/ai-agent", badge: "جديد" },
...
export default function DashboardPage() {
  ...
  // --- API Integrations (Connecting your logic) ---

  // 1. Get User Profile
  const { data: userProfile } = useQuery<UserProfile>({
    queryKey: ['user-profile'],
    queryFn: async () => (await api.get('/auth/users/me/')).data, // Adjusted endpoint for typical Django setup
  });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-50 via-blue-50/50 to-white flex" dir="rtl">
      ...
      <aside className="w-72 bg-white border-l border-gray-200 flex flex-col shadow-sm sticky top-0 h-screen">
        ...
        <nav className="flex-1 px-3 py-2">
          {navItems.map((item) => (
            ...
            >
              ...
              {item.badge && (
                <span className="mr-auto px-2 py-0.5 bg-black text-white text-xs rounded-full" style={{ fontFamily: "'Zain', sans-serif" }}>
                  {item.badge}
                </span>
              )}
            </Link>
        ...
        {/* Bottom Section (Connected to Real Stats) */}
        <div className="p-4 border-t border-gray-100">
          ...
          <div className="mb-4 space-y-3">
            ...
            <div className="w-full bg-gray-200 rounded-full h-1.5">
              <div 
                ...
                // Assuming capacity of 20 for visual representation, modify as needed
            ...
            <div className="flex items-center justify-between text-sm">
              ...
              {/* Display Real Daily Limit Stats */}
            ...
            <div className="w-full bg-gray-200 rounded-full h-1.5">
              <div 
                className={`h-1.5 rounded-full ${userStats?.today_count === userStats?.daily_limit ? 'bg-red-500' : 'bg-blue-600'}`}
          ...
          {!userProfile?.is_premium && (
            <button className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-gradient-to-l from-blue-600 to-blue-500 hover:from-blue-700 hover:to-blue-600 text-white rounded-xl font-medium transition-all shadow-md hover:shadow-lg" style={{ fontFamily: "'Zain', sans-serif" }}>
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="flex-1 flex flex-col min-h-screen">
        {/* Header (Internal Navigation - Keep this as per your request) */}
        <header className="h-16 bg-white/80 backdrop-blur-sm border-b border-gray-100 flex items-center justify-between px-6 sticky top-0 z-40">
          ...
          <div className="flex items-center gap-4">
            ...
            {/* User Menu (Connected to Real Profile) */}
              ...
              {showUserMenu && (
                <div className="absolute left-0 top-full mt-2 w-56 bg-white rounded-xl border border-gray-200 shadow-xl overflow-hidden z-50">
                  <div className="p-4 border-b border-gray-100 bg-gray-50">
                    <p className="font-medium text-black" style={{ fontFamily: "'Zain', sans-serif" }}>{userProfile?.username}</p>
                    <p className="text-sm text-gray-500 truncate">{userProfile?.email}</p>
                  </div>
                  <div className="py-2">
                    <Link href="/settings" className="flex items-center gap-3 px-4 py-2 text-gray-600 hover:text-black hover:bg-gray-50 transition-colors">
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <main className="flex-1 p-6 overflow-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
            <div onClick={() => router.push('/')} className="bg-white border border-gray-200 rounded-2xl p-5 hover:border-blue-300 hover:shadow-md transition-all cursor-pointer group">
              <div className="flex items-start gap-4">
                <div className="w-12 h-12 bg-blue-100 rounded-xl flex items-center justify-center group-hover:bg-blue-200 transition-colors">
                  <SparkleIcon className="w-6 h-6 text-blue-600" />
                </div>
                <div>
                  <h3 className="text-black font-semibold mb-1" style={{ fontFamily: "'Zain', sans-serif" }}>مساعد الذكاء الاصطناعي</h3>
                  <p className="text-gray-500 text-sm" style={{ fontFamily: "'Zain', sans-serif" }}>أقوى أداة لإنشاء السير الذاتية</p>
                </div>
              </div>
            </div>
            
            <div className="bg-white border border-gray-200 rounded-2xl p-5 hover:border-green-300 hover:shadow-md transition-all cursor-pointer group opacity-70">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h3 className="text-black font-semibold mb-1" style={{ fontFamily: "'Zain', sans-serif" }}>البحث عن وظائف</h3>
                  <p className="text-gray-500 text-sm" style={{ fontFamily: "'Zain', sans-serif" }}>قريباً..</p>
                </div>
              </div>
            </div>

            <div className="bg-white border border-gray-200 rounded-2xl p-5 hover:border-purple-300 hover:shadow-md transition-all cursor-pointer group opacity-70">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h3 className="text-black font-semibold mb-1" style={{ fontFamily: "'Zain', sans-serif" }}>مراجعة سيرتي</h3>
                  <p className="text-gray-500 text-sm" style={{ fontFamily: "'Zain', sans-serif" }}>قريباً..</p>
                </div>
              </div>
            </div>
          ...
          <div>
            ...
            {/* Grid */}
            {isProjectsLoading ? (
               <div className="flex justify-center py-20"><Loader2 className="animate-spin w-10 h-10 text-gray-400" /></div>
            ) : (
                <div className={`grid gap-5 ${viewMode === "grid" ? "grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4" : "grid-cols-1"}`}>
                  ...
                  {sortedProjects.map((project) => (
                    <div
                      ...
                      onClick={() => {
                          // Use latest_job_id from serializer
                          if (project.latest_job_id) {
                              router.push(`/cv-editor/${project.latest_job_id}`);
                          } else {
                              // If no job, try creating a new one or handle gracefully
                    ...
                    >
                      ...
                      <div className="relative aspect-[8.5/11] bg-gray-50 overflow-hidden">
                        <Image
                          src="/templates/cv-1.png" // Static for now, can be dynamic
                          alt={project.name}
                          fill
                        ...
                        <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
                          ...
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              if (project.latest_job_id) router.push(`/cv-editor/${project.latest_job_id}`);
                            }}
                          ...
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              if(confirm("هل أنت متأكد من حذف هذه السيرة؟")) deleteMutation.mutate(project.id);
                            }}
